...
1. **`organize_photos.py`** - Organizes photos and videos into date-based folders
2. **`find_duplicates.py`** - Finds likely duplicate files in the same folder
3. **`delete_empty_folders.py`** - Removes junk files and deletes empty folders
...
1. **Organize files** with `organize_photos.py`
2. **Review for duplicates** with `find_duplicates.py`
3. **Clean up leftovers** with `delete_empty_folders.py`

## Included Documentation Files

These files are quick references for each script:

- `organize_photos.md` - script-specific usage notes
- `find_duplicates.md` - duplicate detection details
- `delete_empty_folders.md` - cleanup behavior and confirmations
